<?php
session_start();
if (!isset($_SESSION['logged_in']) || $_SESSION['role'] !== 'asisten_lab') {
...
<head>
  <meta charset="UTF-8" />
  <title>Form Input LPJ Talkshow</title>
...
<body>
  <div class="wrapper">
    ...
    <main class="main-content">
      <div class="content-wrapper">
        <h2 class="form-title">Form Input LPJ Talkshow</h2>

        <form method="POST" action="generate_lpj_talkshow.php" enctype="multipart/form-data" target="hidden_iframe" onsubmit="return validateFormClientSide() && onSubmitForm()">

          <?php
          // Identitas Kegiatan
          input_field('Nama Kegiatan', 'Nama_Kegiatan', 'text', 'Contoh: Talkshow LAB ICT 2025');
          input_field('Judul Kegiatan', 'Judul_Kegiatan', 'text', 'Contoh: Literasi Digital dan Keamanan Siber');
          ...
          input_field('Waktu Kegiatan', 'Waktu_Kegiatan', 'text', 'Contoh: Sabtu, 18 Mei 2025');
          ...
          // Sie Humas
          input_field('Nama Koordinator Sie Humas', 'Nama_Koordinator_Humas', 'select');
          for ($i=1; $i <= 7; $i++) {
              input_field("Nama Anggota Sie Humas {$i}", "Nama_Anggota_Humas_{$i}", 'select', '', false);
          }
          ...
          input_field('Nama Pemateri', 'Nama_Pemateri', 'select');
          input_field('Nama Moderator', 'Nama_Moderator', 'text');

          // MC
          input_field('Nama MC', 'Nama_MC', 'select');

          // Poin Talkshow (textarea)
          textarea_field('Poin Talkshow', 'Poin_Talkshow', 5, 'Jelaskan poin-poin penting talkshow', true);

          // Tema Talkshow
          input_field('Tema Talkshow', 'Tema_Talkshow', 'text', 'Contoh: Literasi Digital dan Keamanan Siber');

          // Poin-Poin Talkshow (textarea)
          textarea_field('Poin-Poin Talkshow', 'Poin_Poin_Talkshow', 5, 'Jelaskan detail poin-poin talkshow', true);

          // Tanya Jawab
          input_field('Penanya 1', 'Penanya_1', 'text', 'Nama penanya 1', false);
          input_field('Pertanyaan 1', 'Pertanyaan_1', 'text', 'Pertanyaan penanya 1', false);
          input_field('Penanya 2', 'Penanya_2', 'text', 'Nama penanya 2', false);
          input_field('Perttanyaan 2', 'Perttanyaan_2', 'text', 'Pertanyaan penanya 2', false);

          // Kesimpulan Talkshow (textarea)
          textarea_field('Kesimpulan Talkshow', 'Kesimpulan_Talkshow', 5, 'Tuliskan kesimpulan talkshow', true);

          // Quiz 1 - 10
          for ($q=1; $q <= 10; $q++) {
              input_field("Quiz Nomor {$q}", "Quiz_Nomor_{$q}", 'text', "Pertanyaan quiz nomor {$q}", false);
          }

          // Juara Quiz 1 - 3
          input_field('Nama Juara 1', 'Nama_Juara_1', 'text', 'Nama juara 1', false);
          input_field('Nama Juara 2', 'Nama_Juara_2', 'text', 'Nama juara 2', false);
          input_field('Nama Juara 3', 'Nama_Juara_3', 'text', 'Nama juara 3', false);

          // Waktu Acara Bermula & Berakhir
          input_field('Waktu Acara Bermulai', 'Waktu_Acara_Bermulai', 'time', '', true);
          input_field('Waktu Acara Berakhir', 'Waktu_Acara_Berakhir', 'time', '', true);

          // Sertifikat
          input_field('Sertifikat Pemateri (file)', 'Sertifikat_Pemateri', 'file', '', false);
          input_field('Sertifikat Moderator (file)', 'Sertifikat_Moderator', 'file', '', false);
          input_field('Sertifikat Panitia (file)', 'Sertifikat_Panitia', 'file', '', false);
          input_field('Sertifikat Peserta (file)', 'Sertifikat_Peserta', 'file', '', false);
          ?>

          <div class="d-grid gap-2">
            <button type="submit" class="btn btn-submit-custom mt-3">Generate dan Kirim LPJ Talkshow</button>
          </div>
        </form>
      </div>
    </main>
  ...
  function onSubmitForm() {
    setTimeout(() => {
      alert('LPJ Talkshow berhasil digenerate dan dikirim!');
    }, 1500);
  }
  </script>
